...
class SchedulingBudget:
    ...
    _num_batched_tokens_actual: int = 0
    ...
    def add_num_batched_tokens(
        ...
        num_actual_batched_tokens: Optional[int] = None,
    ):
        ...
        if num_actual_batched_tokens is None:
            num_actual_batched_tokens = num_batched_tokens
        self._num_batched_tokens_actual += num_actual_batched_tokens
    ...
    @property
    def num_batched_tokens_actual(self):
        return self._num_batched_tokens_actual
...
class SchedulerOutputs:
    ...
    num_batched_tokens: int
    num_batched_tokens_from_budget: int
...
class Scheduler:
    ...
    def _schedule_prefills(
        ...
        while self._passed_delay(time.time()) and waiting_queue:
            # Don't schedule any prefill if 90% of the blocks are occupied.
            if (
                self.block_manager.get_num_free_gpu_blocks()
                < 0.1 * self.block_manager.num_total_gpu_blocks
            ):
                break
            ...
            num_new_tokens_exclude_cached = (
                num_new_tokens - waiting_seqs[0].get_num_cached_tokens()
            )
            ...
            num_new_seqs = seq_group.get_max_num_running_seqs()
            if num_new_tokens_exclude_cached > 0:
    ...
    def _schedule_default(self) -> SchedulerOutputs:
        ...
        return SchedulerOutputs(
            ...
            num_batched_tokens=budget.num_batched_tokens_actual,
