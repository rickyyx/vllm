...
from .characterlevelparser import (CharacterLevelParser,
                                   ...
                                   JsonEscapingParser, SequenceParser,
                                   StringParser, UnionParser)
from .consts import (BACKSLASH, BACKSLASH_ESCAPING_CHARACTERS,
                     MAX_CONSECUTIVE_WHITESPACES, WHITESPACE_CHARACTERS)
...
_ANY_JSON_SCHEMA_DICT = {
    'anyOf': [{
        'type': type
    } for type in json_schema_data_formats.keys() if type != 'integer']
}


def _can_shortcut_string_parsing_state(
        string_parsing_state: "StringParsingState") -> bool:
    return not string_parsing_state.allowed_strings and not string_parsing_state.regex_parser


class JsonSchemaParser(CharacterLevelParser):
    ANY_JSON_OBJECT_SCHEMA: JsonSchemaObject = JsonSchemaObject(
        **_ANY_JSON_SCHEMA_DICT)
    ...
    def add_character(self, new_character: str) -> CharacterLevelParser:
        ...
        while new_character not in self.object_stack[
                receiving_idx].get_allowed_characters():
        ...
        updated_parser = JsonSchemaParser(self.context, self.config,
                                          updated_stack,
                                          self.num_consecutive_whitespaces)
        updated_parser.context.active_parser = updated_parser
        updated_parser.last_parsed_string = last_parsed_string
        updated_parser.object_stack[
            receiving_idx] = updated_parser.object_stack[
                receiving_idx].add_character(new_character)
        ...
        if isinstance(updated_parser.object_stack[-1], UnionParser) and \
            ...
            for option_parser in updated_parser.object_stack[-1].parsers:
                option_stack = updated_parser.object_stack[:-1] + [
                    option_parser
                ]
                option_parser = JsonSchemaParser(
                    self.context, self.config, option_stack,
        ...
        # For some performance optimizations to work, we want to make
        # sure we don't leave irrelevant objects at the top of the stack,
        # which we know will be passed over next timestep
        new_object_stack = updated_parser.object_stack
        while new_object_stack and new_object_stack[-1].can_end(
        ) and new_object_stack[-1].get_allowed_characters() == '':
    ...
    def get_allowed_characters(self) -> str:
        ...
        if self.num_consecutive_whitespaces >= MAX_CONSECUTIVE_WHITESPACES:
            ...
            allowed_characters = "".join(c for c in allowed_characters
                                         if c not in WHITESPACE_CHARACTERS)
    ...
    def shortcut_key(self) -> Optional[Hashable]:
        if self.object_stack:
            ...
            # If we are in a string parsing state, we can shortcut since
            # everything is almost possible
            if isinstance(current_parser, StringParsingState):
                string_parsing_state = current_parser
            elif isinstance(current_parser, UnionParser):
                # It's possible that StringParsing is part of the
                # AnyOf / UnionParser that is the top parser at the moment. We
                # also need to check for this case.
                union_string_parsing_state = next(
                    (parser for parser in current_parser.parsers
                     if isinstance(parser, StringParsingState)
                     and _can_shortcut_string_parsing_state(parser)), None)
                if union_string_parsing_state:
                    string_parsing_state = union_string_parsing_state
                ...
                if isinstance(prev_parser, StringParsingState):
                    ...
                    elif isinstance(current_parser, JsonEscapingParser):
                        # Another way to shortcut is if we are in a backslash
                        # state, because for some reason everything is also
                        # possible after a backslash
                        backslash_state = current_parser

            if backslash_state:
                return ('json_backslash', )
                ...
                if _can_shortcut_string_parsing_state(
                        string_parsing_state
                ) and string_parsing_state.seen_opening_quote and not string_parsing_state.seen_closing_quote:
                    # Performance optimization: When we are parsing a string that is not from a list of allowed strings, most tokens
                    # are legal. The exploration can be more costly than the LM itself for large tokenizers (because this is pure python),
                    # so we signal that we are in a "freetext" mode, and reuse the allowed token list throughout the run.
                    cur_len = len(string_parsing_state.parsed_string)
                    min_len = string_parsing_state.min_length or 0
                    max_len = string_parsing_state.max_length or sys.maxsize
...
def get_parser(parsing_state: JsonSchemaParser,
               value_schema: JsonSchemaObject) -> CharacterLevelParser:
    if value_schema is None:
        raise LMFormatEnforcerException(
            "JsonSchemaParser: Value schema is None")
    if value_schema.anyOf:
        parsers = [
            get_parser(parsing_state, schema) for schema in value_schema.anyOf
        ]
        # Deduplicate. Note we cannot use a set here because the
        # parser objects are not hashable.
        filtered_parsers = []
        for parser in parsers:
            if parser not in filtered_parsers:
                filtered_parsers.append(parser)
        # If we have only one parser, we can use that directly
        if len(filtered_parsers) == 1:
            return filtered_parsers[0]
        return UnionParser(filtered_parsers)
    if value_schema.oneOf or value_schema.allOf:
        raise LMFormatEnforcerException(
            "allOf and oneOf fields are not yet supported.")
    if value_schema.type == "string":
        is_string = all(isinstance(i, (str)) for i in value_schema.enum)
        if not is_string:
            raise LMFormatEnforcerException(
                f"Unsupported enum type {value_schema.enum} for string field. "
        ...
        if 'definitions' in extras:
            ...
            raise LMFormatEnforcerException("No definitions found in schema")
        ...
        is_numeric = all(
            isinstance(i, (int, float)) for i in value_schema.enum)
        is_string = all(isinstance(i, (str)) for i in value_schema.enum)
        if is_string:
            return StringParsingState(
                parsing_state,
            ...
            raise LMFormatEnforcerException(
                f"Unsupported enum type {value_schema.enum}. "
                "Make sure all enum values are all strings or all numeric.")
        ...
        return ListParsingState(parsing_state, item_schema,
                                value_schema.minItems, value_schema.maxItems)
    else:
        raise LMFormatEnforcerException("Unsupported type " +
                                        str(value_schema.type))
...
class ObjectParsingState(BaseParsingState):
    ...
    def __init__(self, schema_object: JsonSchemaObject,
    ...
    def add_character(self, new_character: str) -> CharacterLevelParser:
        ...
        if (self.current_stage == ObjectParsingStage.START_OBJECT
            ...
            if new_character == '"':
                ...
                key_parser = StringParsingState(self.root,
                                                possible_keys,
                                                require_opening_quote=True,
                                                require_closing_quote=True)
            ...
            if new_character == ":":
                ...
                if self.is_dictionary:
                    ...
                    possible_keys = list(
                        set(possible_keys).difference(self.existing_keys))
                    value_schema = self.schema_object.properties[
                        self.current_key]
                self.current_key_parser = get_parser(self.root, value_schema)
                self.root.context.active_parser.object_stack.append(
                    self.current_key_parser)
                self.current_key_parser = None
        elif self.current_stage == ObjectParsingStage.PARSING_VALUE:
            # If we receive a character during parsing value, it means that its the finishing character
            # of the value parser
            if new_character == '"':
    ...
    def get_allowed_characters(self) -> str:
        possible_keys = (list(self.schema_object.properties.keys())
                         if not self.is_dictionary else None)
...
class PrimitiveParsingState(BaseParsingState):
    ...
    def __eq__(self, __value: object) -> bool:
        return isinstance(__value,
                          type(self)) and self.__dict__ == __value.__dict__
...
class NumberParsingState(PrimitiveParsingState):
    ...
    def can_end(self) -> bool:
        return bool(
            self.parsed_string) and (self.parsed_string[-1].isdigit()
                                     or self.seen_whitespace_after_digits)
...
class StringParsingState(PrimitiveParsingState):
    ...
    def __init__(
        ...
        if self.pattern and (self.min_length or self.max_length):
            raise LMFormatEnforcerException(
                "String schema contains both a pattern and a min/max length, which is not currently supported"
            )
        ...
        if self.pattern and not regex_parser:
            if self.pattern not in self.root.context.regex_parser_cache:
                self.root.context.regex_parser_cache[
                    self.pattern] = RegexParser(self.pattern, self.root.config)
            self.regex_parser = self.root.context.regex_parser_cache[
                self.pattern]

    def _clone(self) -> "StringParsingState":
        clone = StringParsingState(self.root, self.allowed_strings,
                                   self.require_opening_quote,
                                   self.require_closing_quote, self.min_length,
                                   self.max_length, self.pattern,
                                   self.regex_parser)
        clone.parsed_string = self.parsed_string
    ...
    def add_character(self, new_character: str):
        if (not self.parsed_string or self.seen_closing_quote
            ) and new_character in WHITESPACE_CHARACTERS:
            return self
    ...
    def get_allowed_characters(self) -> str:
        ...
        if self.allowed_strings:
            allowed_continuations = [
                s[len(self.parsed_string):] for s in self.allowed_strings
                if s.startswith(self.parsed_string)
            ]
            allowed_next_characters = [
                allowed_continuation[0]
                for allowed_continuation in allowed_continuations
                if len(allowed_continuation) > 0
            ]
            ...
            if (not self.parsed_string) and (not self.seen_opening_quote or
                                             not self.require_opening_quote):
                allowed_next_characters.extend(WHITESPACE_CHARACTERS)
            return "".join(allowed_next_characters)
        else:
            if self.min_length is not None and len(
                    self.parsed_string) < self.min_length:
                return self.root.context.alphabet_without_quotes + BACKSLASH
            if self.max_length is not None and len(
                    self.parsed_string) >= self.max_length:
                return '"'
...
class ListParsingState(PrimitiveParsingState):
    ...
    def _clone(self) -> PrimitiveParsingState:
        new = ListParsingState(self.root, self.list_member_type,
        ...
        if new_character == "[":
            ...
            if requires_items:
                ...
                if isinstance(item_parser, UnionParser):
                    ...
                    parser_to_push = UnionParser(
                        [item_parser, empty_list_parser])
    ...
    def get_allowed_characters(self) -> str:
        if not self.seen_list_opener:
            ...
            return self.get_allowed_control_characters(
    ...
    def get_allowed_control_characters(self):
        ...
        if (
                not is_on_top
        ) and self.root.context.active_parser.last_non_whitespace_character != "[":
