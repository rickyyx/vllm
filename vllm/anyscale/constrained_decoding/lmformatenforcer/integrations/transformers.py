...
try:
    ...
    from transformers.generation.logits_process import (
        LogitsWarper, PrefixConstrainedLogitsProcessor)
...
try:
    ...
    raise ImportError(
        'pytorch is not installed. See https://pytorch.org/get-started/locally/ for installation instructions."'
    )
...
class LogitsSaverWarper(LogitsWarper):
    ...
    def __call__(self, input_ids: torch.LongTensor,
        ...
        for single_batch_inputs, single_batch_scores in zip(
                cpu_inputs, cpu_scores):
            self.analyzer.report_raw_logits(single_batch_inputs,
                                            single_batch_scores)
        return scores
...
class LogitsSaverManager:
    ...
    def __init__(self, model: AutoModelForCausalLM,
                 analyzer: FormatEnforcerAnalyzer):
        self.model = model
        self.warper = None
...
def _build_regular_tokens_list(
        tokenizer: PreTrainedTokenizerBase) -> List[Tuple[int, str, bool]]:
...
class TransformersPrefixAllowedTokensFn:
    ...
    def __call__(self, batch_id: int, sent: torch.Tensor) -> List[int]:
        ...
        return list(self.token_enforcer.get_allowed_tokens(token_sequence))
...
def build_transformers_prefix_allowed_tokens_fn(
    ...
    if isinstance(tokenizer_data, PreTrainedTokenizerBase):
        tokenizer_data = deepcopy(tokenizer_data)
...
def generate_enforced(model: AutoModelForCausalLM,
    ...
    transformers_filter_allowed_tokens = build_transformers_prefix_allowed_tokens_fn(
        tokenizer, character_level_parser)

    is_multi_inputs = kwargs['input_ids'].shape[0] > 1
    is_multi_beams = kwargs.get('num_beams', 1) > 1
    support_diagnostics = not (
        is_multi_inputs or is_multi_beams
    )  # TODO: Support diagnostics in these cases as well.
    ...
    if should_run_in_advanced_mode:
        analyzer = FormatEnforcerAnalyzer(
            transformers_filter_allowed_tokens.token_enforcer)
        ...
        df_dict = analyzer.generate_report_dict(
            output['sequences'][0].tolist())
        output.enforced_scores = df_dict
    else:
        output = model.generate(
            **kwargs,
            prefix_allowed_tokens_fn=transformers_filter_allowed_tokens)
