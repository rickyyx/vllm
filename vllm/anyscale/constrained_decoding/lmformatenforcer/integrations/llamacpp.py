...
def _build_regular_tokens_list(llm: Llama) -> List[Tuple[int, str, bool]]:
    ...
    for token_idx in range(llm.n_vocab()):
        ...
        try:
            decoded_after_0 = llm.detokenize([token_0,
                                              token_idx]).decode('utf-8')[1:]
...
def build_token_enforcer_tokenizer_data(
        llm: Llama) -> TokenEnforcerTokenizerData:
    ...
    return TokenEnforcerTokenizerData(regular_tokens, decoder_fn,
                                      llm.token_eos())
...
class LlamaCppLogitsProcessor:
    ...
    def __call__(self, input_ids: npt.NDArray[np.intc],
                 scores: npt.NDArray[np.single]) -> npt.NDArray[np.single]:
...
def build_llamacpp_logits_processor(
        llm: Union[Llama, TokenEnforcerTokenizerData],
        character_level_parser: CharacterLevelParser,
        analyze: bool = False) -> LlamaCppLogitsProcessor:
