...
try:
    ...
    raise ImportError(
        'haystack is not installed. Please install it with "pip install farm-haystack" or "pip install haystack-ai'
    )
...
class LMFormatEnforcerLocalGenerator:
    ...
    def _prepare_token_enforcer_fn(self) -> Optional[Callable]:
        ...
        raise NotImplementedError(
            f"Token enforcer not implemented for model type {self._model_type.name}"
        )
    ...
    def _resolve_model_type(self) -> _ModelType:
        ...
        try:
            ...
            raise ValueError(
                f"Unsupported local generator component layer: {generator_component_name}. "
    ...
    def _inject_enforcer_into_model(self):
        if not self.token_enforcer_fn:
            return
        if self._model_type == _ModelType.HUGGINGFACE:
            self.model_component.generation_kwargs[
    ...
    def _release_model_injection(self):
        if not self.token_enforcer_fn:
            return
        if self._model_type == _ModelType.HUGGINGFACE:
            del self.model_component.generation_kwargs[
