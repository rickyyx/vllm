...
from .characterlevelparser import JsonEscapingParser
...
class JsonFreetextTokenCache:
    ...
    class _StringLengthTokenCache:
        ...
        def build(self, token_strs_to_idx: List[Tuple[str, int]]):
            ...
            token_strs_to_idx = sorted(token_strs_to_idx,
        ...
        def get_indices_between_length(self,
                                       min_length=-1,
                                       max_length=-1) -> List[int]:
            if min_length >= len(self.first_index_geq_than_length):
                return []
            start_index = self.first_index_geq_than_length[
                min_length] if min_length > 0 else 0
            if max_length == 0:
                end_index = 0
    ...
    def lookup_allowed_tokens(self, min_remaining: int,
        ...
        if cache_key not in self.allowlist_cache:
            tokens_with_quote = self.quote_tokens_length_cache.get_indices_between_length(
                min_remaining + 1, max_len + 1)
            tokens_without_quote = self.regular_tokens_length_cache.get_indices_between_length(
                -1, max_len)
    ...
    def freeze(self) -> None:
        ...
        all_tokens: List[Tuple[str, int]] = list(
            (s, n) for n, s in self.token_num_to_str.items())
        assert all_tokens, "Cannot precalculate allowlists for an empty token list"
        assert not any(
            pair[0] == ''
            for pair in all_tokens), "Tokenizer must not contain empty tokens"
        ...
        self.max_token_len = max(
            len(self.regular_tokens_length_cache.first_index_geq_than_length),
            len(self.quote_tokens_length_cache.first_index_geq_than_length))
        del self.token_num_to_str


class BackslashTokensCache:
    ...
    def add_token(self, token_str: str, token_int: int):
        ...
        evaluated_len = 0
        if token_str[0] != "u" and '"' not in token_str[1:]:
            for character in token_str:
                allowed_characters = json_escaping_parser.get_allowed_characters(
                )
                if character in allowed_characters:
                    json_escaping_parser = json_escaping_parser.add_character(
                        character)
                else:
                    break
                evaluated_len += 1
    ...
    def lookup_allowed_tokens(self) -> Tuple[int, ...]:
        """
        Get the list of tokens that are allowed within a JSON string, such that:
        1. all candidate tokens are at most `max_len` characters long (excluding the trailing quote), and
        2. if a token ends with a quote, it's at least `min_remaining` chars long (excluding the quote).
        """
        return tuple(self.token_num_to_str.keys())
...
class TokenizerPrefixTree:
    ...
    def __init__(self, regular_tokens: List[Tuple[int, str, bool]]):
        ...
        self.backslash_tokens = BackslashTokensCache()
        self.new_word_tokens: Set[int] = set()
        self.tokens_to_strs = {
            token_idx: token_str
            for token_idx, token_str, _ in regular_tokens
        }
        for token_idx, decoded, is_new_word in regular_tokens:
            ...
            self.backslash_tokens.add_token(decoded, token_idx)
            if is_new_word:
                self.new_word_tokens.add(token_idx)

        self.json_freetext_tokens.freeze()

    def _add_token_to_tree(self, token_str: str, token_idx: int,
