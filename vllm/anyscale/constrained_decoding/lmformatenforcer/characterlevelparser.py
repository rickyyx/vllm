...
from .consts import (BACKSLASH_ESCAPING_CHARACTERS, COMPLETE_ALPHABET,
...
class StringParser(CharacterLevelParser):
    ...
    def add_character(self, new_character: str) -> CharacterLevelParser:
        if self.target_str.startswith(new_character):
            ...
            raise ValueError(
                f"Expected '{self.target_str[0]}' but got '{new_character}'")
...
class UnionParser(CharacterLevelParser):
    ...
    def add_character(self, new_character: str) -> CharacterLevelParser:
        ...
        next_parsers = [
            parser.add_character(new_character) for parser in relevant_parsers
        ]
    ...
    def get_allowed_characters(self) -> str:
        allowed = "".join(
            [parser.get_allowed_characters() for parser in self.parsers])
    ...
    def shortcut_key(self) -> Optional[str]:
        return self.parsers[0].shortcut_key() if len(
            self.parsers) == 1 else None
...
class JsonEscapingParser(CharacterLevelParser):
    ...
    def __init__(self):
        ...
        escaping_character_parsers: List[CharacterLevelParser] = [
            StringParser(c) for c in BACKSLASH_ESCAPING_CHARACTERS
        ]
        hex_digit_parser: CharacterLevelParser = UnionParser(
            [StringParser(c) for c in "0123456789abcdefABCDEF"])
        unicode_components: List[CharacterLevelParser] = list(
            [StringParser("u")] + [hex_digit_parser] * 4)
        unicode_escape_parser: CharacterLevelParser = SequenceParser(
            unicode_components)
        self.json_escaping_parser = UnionParser(escaping_character_parsers +
