...
try:
    ...
    raise ImportError(
        'FormatEnforcerAnalyzer not available because numpy is not installed. Please install it with "pip install numpy"'
    ) from e
...
class FormatEnforcerAnalyzer:
    ...
    def report_raw_logits(self, output_tokens: List[int],
    ...
    def generate_report_dict(self, output_tokens: List[int]) -> dict:
        ...
        for idx in range(len(output_tokens)):
            ...
            if prefix_tuple in self.raw_logits:
                ...
                allowed_tokens_matrix.append(
                    self.token_enforcer.get_allowed_tokens(prefix))
        ...
        original_scores = _select_array(softmax_logits,
                                        original_indices)  # n_tokens

        single_token_dict: Dict[int, str] = {
            token_id: token_str
            for token_id, token_str, _ in self.token_enforcer.regular_tokens
        }
        ...
        original_tokens = [
            single_token_decoder(idx) for idx in original_indices
        ]
        ...
        enforced_scores = _select_array(enfored_softmax_logits,
                                        enforced_indices)

        enforced_tokens = [
            single_token_decoder(idx) for idx in enforced_indices
        ]
        df_dict = {
        }  # In order to minimize the package's dependencies, we don't create a dataframe, but create a dataframe-like dictionary instead.
