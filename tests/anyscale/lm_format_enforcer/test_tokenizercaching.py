...
from vllm.anyscale.constrained_decoding.lmformatenforcer.tokenizerprefixtree import (
...
def test_json_freetext_cache():
    ...
    def _assert_allowed_tokens(_min_remaining, _max_length,
                               _num_expected_tokens):
        allowed_tokens = cache.lookup_allowed_tokens(_min_remaining,
                                                     _max_length)
        if len(allowed_tokens) != _num_expected_tokens:
            allowed_token_strs = "|".join(token_to_str[token_idx]
                                          for token_idx in allowed_tokens)
            raise Exception(
                f"Min={_min_remaining}, Max={_max_length}, Expected {_num_expected_tokens}, got {len(allowed_tokens)} : {allowed_token_strs}"
            )

    for min_remaining in range(0, test_length):
        for max_length in range(min_remaining, test_length):

            num_expected_quote_tokens = num_letters * (max_length -
                                                       min_remaining + 1)
            if min_remaining == 0:
