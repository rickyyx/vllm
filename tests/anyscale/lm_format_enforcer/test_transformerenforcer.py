...
from vllm.anyscale.constrained_decoding.lmformatenforcer import RegexParser
from vllm.anyscale.constrained_decoding.lmformatenforcer.integrations.transformers import (
    build_transformers_prefix_allowed_tokens_fn, generate_enforced)


def _build_pipeline():
    return pipeline('text-generation',
                    model='hf-internal-testing/tiny-random-GPTNeoModel')
...
def test_transfomers_generate_enforced_integration():
    ...
    outputs = generate_enforced(hf_pipeline.model, hf_pipeline.tokenizer,
                                parser, **inputs)
    for idx in range(len(prompts)):
        output_text = hf_pipeline.tokenizer.decode(
            outputs[idx], skip_special_tokens=True)[len(prompts[idx]):]
        assert output_text == 'abc123'
...
def test_transfomers_generate_function_integration():
    ...
    prefix_function = build_transformers_prefix_allowed_tokens_fn(
        hf_pipeline.tokenizer, parser)
    outputs = hf_pipeline.model.generate(
        **inputs, prefix_allowed_tokens_fn=prefix_function)
    for idx in range(len(prompts)):
        output_text = hf_pipeline.tokenizer.decode(
            outputs[idx], skip_special_tokens=True)[len(prompts[idx]):]
        assert output_text == 'abc123'
