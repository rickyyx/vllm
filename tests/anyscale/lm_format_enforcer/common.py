...
from vllm.anyscale.constrained_decoding.lmformatenforcer.exceptions import (
    LMFormatEnforcerException)
from vllm.anyscale.constrained_decoding.lmformatenforcer.tokenenforcer import (
...
def assert_parser_with_string_direct(string: str, parser: CharacterLevelParser,
                                     expect_success: bool):
    for idx, character in enumerate(string):
        try:
            if character in parser.get_allowed_characters():
                ...
                if expect_success:
                    raise CharacterNotAllowedException(
                        f"Parser does not allow '{character}' at index {idx}")
                else:
                    return  # Success
        except LMFormatEnforcerException:
            raise
        except Exception as e:
            raise Exception(f"Error parsing '{character}' at index {idx}: {e}",
                            e)
...
def assert_parser_with_string_token_enforcer(string: str,
                                             parser: CharacterLevelParser,
                                             expect_success: bool,
                                             profile_file_path: Optional[str]):
    global _tokenizer
    ...
    for prefix_length in range(len(initial_token_array),
                               len(target_token_array) + 1):
        ...
        if prefix_length < len(target_token_array):
            ...
            if next_token not in allowed_tokens:
                if expect_success:
                    decoded_before = _tokenizer.decode(
                        prefix, skip_special_tokens=True)
                    decoded_after = _tokenizer.decode(prefix + [next_token],
                                                      skip_special_tokens=True)
                    next_token_chars = decoded_after[len(decoded_before):]
                    next_idx = len(decoded_before) - len(prompt)
                    raise CharacterNotAllowedException(
                        f"Parser does not allow '{next_token_chars}' at index {next_idx}"
                    )
                else:
                    return  # Test success
        else:
            ...
            if not can_end and expect_success:
                raise ValueError(
                    "Parser did not reach end state when it should have")

    if profiler and profile_file_path:
        finish_profiling(profiler, profile_file_path)


def assert_parser_with_string(string: str,
                              parser: CharacterLevelParser,
                              expect_success: bool,
