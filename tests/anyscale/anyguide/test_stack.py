from anyguide import ElementType, Stack
...
class TestStackInitialization:
    ...
    def test_initialization_with_iterators(self):
        """Test initializing a stack with iterators."""
        elements = [
            create_element(code_point=ord('a')),
            create_element(code_point=ord('b')),
            create_element(code_point=ord('c'))
        ]
        stack = Stack(iter(elements))
        assert not stack.is_empty()
        assert stack.size() == 3
...
class TestStackAdvancedOperations:
    ...
    def test_getitem_slice_normal(self):
        ...
        assert sub_stack[0].code_point == 9
        assert sub_stack[1].code_point == 8
        assert sub_stack[2].code_point == 7
    ...
    def test_getitem_slice_skip_top_element(self):
        ...
        assert sub_stack.top().code_point == 8
