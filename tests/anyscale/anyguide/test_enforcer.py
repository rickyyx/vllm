...
import pytest
import torch
from anyguide import Grammar, GrammarEnforcer, PrefixCache, TokenizerTrie
from anyguide.json_schema_to_grammar import (HEX_DIGIT, SPACE_END, SPACE_RULE,
                                             STRING)
...
# Used to be non-empty on python implementation
EXCLUDED_PERFORMANCE_TESTS = {}
...
class TestEnforcerMicroBenchmark:

    def test_walk_trie_performance(self, tokenizer_trie):
        grammar = Grammar.from_str("root ::= [a-z0-9A-Z?]*")
        ge = GrammarEnforcer(grammar=grammar, tokenizer_trie=tokenizer_trie)
        ge.init()
        s = time.time()
        mask = ge.get_tokens_mask()
        print(f"{mask.sum()=}")
        print(f"Time taken to walk trie: {(time.time() - s):.2f} sec")
